...
async function checkNumberOfTodosInLocalStorage(page: Page, expected: number) {
  return await page.waitForFunction(e => {
    return JSON.parse(localStorage['react-todos'] as unknown as never).length === e;
  }, expected);
}
...
async function checkNumberOfCompletedTodosInLocalStorage(page: Page, expected: number) {
  return await page.waitForFunction(e => {
    return (
      JSON.parse(localStorage['react-todos']).filter((todo: { title: string; completed: boolean }) => todo.completed)
        .length === e
    );
  }, expected);
}
